<template>     
    <VForm
      ...
      <div class="row">
        <div class="col-12">
          <ElementsFormVTextInput type="text" name="nameEn" id="nameEn" label="Company Name" placeholder="Company Name" astricts="true" />
        </div>
        <div class="col-sm-6">
          <ElementsFormVTextInput type="email" name="email" id="email" label="Email" placeholder="Email" astricts="true" />
        </div>
        <div class="col-sm-6">
          <ElementsFormVTextInput type="number" name="officePhone" id="officePhone" label="phone" placeholder="phone" astricts="true" />
        </div>
        <div class="col-sm-6">
        ...
        <div class="col-sm-6">
          <ElementsFormVTextInput type="text" name="responsiblePersonNameEn" id="responsiblePersonNameEn" label="Responsible Person Name" placeholder="Responsible Person Name" />
        </div>
        <div class="col-sm-6">
          <ElementsFormVTextInput type="number" name="responsiblePersonMobile" id="responsiblePersonMobile" label="Responsible Person Mobile" placeholder="Responsible Person Mobile" />
        </div>
        <div class="col-6">
...
<script setup>
  ...
  const handleSubmit = async (values, actions) => {
    ...
    values.nameAr = values.nameEn;
    values.responsiblePersonNameAr = values.responsiblePersonNameEn;
    values.addressAr = values.addressEn;
    const { data, error } = await useFetch(api.CompanyClientsApi, {
      baseURL: apiBase,
      method: 'POST',
      body: values,
      headers: {
  ...
  const schema = object({
    ...
    officePhone: string().required().min(3).label("Your phone"),
